...
function getClass(classname,father){
	...
	if(father.getElementsByClassName){//存在class名
		return father.getElementsByClassName(classname);//返回
	}else{//如果存在多个class名
		var all=father.getElementsByTagName("*");//获取全部的选择器
		var newarr=[];
		for(var i in all){
			if(clickPre(all[i].className,classname)){//对比选择器的所有class名
				newarr.push(all[i]);
			}
		}
		return newarr;//返回全部
...
function $(selecter,father){
	if(typeof selecter=="string"){
		...
		if(selecter.charAt(0)=="."){//如果有.
			return getClass(selecter.slice(1),father);
		}else if(selecter.charAt(0)=="#"){//如果有#
			return document.getElementById(selecter.slice(1));
		}else if(/^[a-z]+\d*$/g.test(selecter)){//如果是选择器名
			return father.getElementsByTagName(selecter);
		}else if(/^<[a-z]+\d*>$/g.test(selecter)){//如果加有标签对
			return document.createElement(selecter.slice(1,-1));//去前后解析
		}
